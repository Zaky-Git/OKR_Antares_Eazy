...
export function KeyResultPanel({ keyResult, objectiveId, onClose }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: FormData) => {
      ...
      return keyResultService.create(objectiveId, payload);
    },
    onSuccess: () => {
    ...
    <div className="space-y-0">
      ...
      <form onSubmit={handleSubmit((data) => saveMutation.mutate(data))} className="space-y-4">
        <div>
          ...
          <input {...register('title', { required: 'Wajib diisi' })} placeholder="Contoh: Meningkatkan DAU dari 7,500 ke 10,000" className="w-full px-3.5 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10" />
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="number" step="any" {...register('target_value', { required: 'Wajib' })} className="w-full px-3.5 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10" />
          </div>
          <div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          ...
          <button type="submit" disabled={saveMutation.isPending} className="px-5 py-2.5 text-sm font-semibold text-white bg-primary rounded-lg hover:bg-primary-hover disabled:opacity-50 transition-colors">
